...
void ReadAndWriteFile::execute(string inputFileName,string outputFileName1 , string outputFileName2) {
    ...
    Model1* model1 = new Model1(); // AVL Tree Model
    Model2* model2 = new Model2(); // LLRBL Tree Model

    ModelCommon* modelCommon = new ModelCommon(); // Binary Search Tree
    ...
    while(getline(inputFile,inputString)){ // File Reading loop

        string innerString;
        istringstream innerStream(inputString);
        vector<string> tempVector;

        while(getline(innerStream , innerString,'\t')) { // inner loop with delim tab
